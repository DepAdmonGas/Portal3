...
class GestoriaPermisosController extends BaseController
{
    ...
    private function requiereAtencion(
        ?string $vigencia,
        ?string $fechaVencimiento
    ): bool {
        if (
            $vigencia === 'Cuando se realice cambio' ||
            $vigencia === 'Permanente'
        ) {
            return false;
        }
        ...
        try {
            $fechaNotificacion = Carbon::parse($fechaVencimiento)
                ->subDays(30);

            return $fechaNotificacion->lte(Carbon::today());
        } catch (\Throwable) {
            return false;
        }
    }
