...
Alpine.data('modulosUsuariosForm', () => ({
...
init() {
window.modulosUsuarioInstance = this;
...
let url = `/configuracion/modulos-usuarios/modulo/${id}`;
if (currentId) {
url += `/${currentId}`;
}
...
async openEditar(id){
try {
const res = await fetch(`/configuracion/modulos-usuarios/detalle/${id}`);
...
let url = '/configuracion/modulos-usuarios/create';

if (this.mode === 'edit') {
url = `/configuracion/modulos-usuarios/update/${this.editId}`;
}
