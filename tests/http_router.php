...
if (!$schema->hasTable('tb_usuarios')) {
    ...
    $schema->create('op_solicitud_cheque', static function ($table): void { $table->integer('id')->primary(); $table->integer('id_estacion'); $table->integer('id_year')->default(2026); $table->integer('id_mes')->default(9); $table->integer('status')->default(0); });
    $schema->create('op_solicitud_cheque_documento', static function ($table): void { $table->integer('id')->primary(); $table->integer('id_solicitud'); $table->string('nombre'); });
    $schema->create('rl_requisitos_legales_calendario', static function ($table): void { $table->integer('id')->primary(); $table->integer('id_estacion'); });
    $schema->create('rl_requisitos_legales_matriz', static function ($table): void { $table->integer('id')->primary(); $table->integer('idcalendario'); $table->string('acusepdf')->nullable(); $table->string('requisitolegalpdf')->nullable(); });
    ...
    Capsule::table('tb_modulos_config')->insert(['id' => 2, 'modulo_key' => 'solicitud-cheque', 'tipo' => 'stations', 'estaciones_soportadas' => '[101,202]', 'tipo_departamento' => 'puestos', 'activo' => 1]);
    Capsule::table('tb_modulos_config')->insert(['id' => 3, 'modulo_key' => 'sgm', 'tipo' => 'stations', 'estaciones_soportadas' => '[101]', 'tipo_departamento' => 'puestos', 'activo' => 1]);
    Capsule::table('tb_modulos_config')->insert(['id' => 4, 'modulo_key' => 'sasisopa', 'tipo' => 'stations', 'estaciones_soportadas' => '[101]', 'tipo_departamento' => 'puestos', 'activo' => 1]);
    Capsule::table('op_solicitud_cheque')->insert([['id' => 11, 'id_estacion' => 101], ['id' => 22, 'id_estacion' => 202]]);
    Capsule::table('op_solicitud_cheque_documento')->insert([['id' => 1, 'id_solicitud' => 11, 'nombre' => 'A'], ['id' => 2, 'id_solicitud' => 22, 'nombre' => 'B']]);
    Capsule::table('rl_requisitos_legales_calendario')->insert([['id' => 301, 'id_estacion' => 101], ['id' => 302, 'id_estacion' => 202]]);
    Capsule::table('rl_requisitos_legales_matriz')->insert([['id' => 401, 'idcalendario' => 301, 'acusepdf' => 'sgm-a-acuse.pdf', 'requisitolegalpdf' => 'sgm-a-requisito.pdf'], ['id' => 402, 'idcalendario' => 302, 'acusepdf' => 'sgm-b-acuse.pdf', 'requisitolegalpdf' => 'sgm-b-requisito.pdf'], ['id' => 403, 'idcalendario' => 301, 'acusepdf' => null, 'requisitolegalpdf' => '../outside.pdf'], ['id' => 404, 'idcalendario' => 999, 'acusepdf' => 'sgm-a-acuse.pdf', 'requisitolegalpdf' => 'sgm-a-requisito.pdf'], ['id' => 405, 'idcalendario' => 301, 'acusepdf' => 'sasisopa-a-acuse.pdf', 'requisitolegalpdf' => 'sasisopa-a-requisito.pdf']]);
}

Session::init();
$path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
if ($path === '/__test/health') exit('ok');
if ($path === '/__test/login-a') { Auth::forget(); Session::set('usuario', ['id' => 1, 'id_estacion' => 101, 'nombre_estacion' => 'Tenant A', 'razonsocial' => 'A SA', 'multiestacion' => true]); Session::set('module_context', ['sgm' => ['id_estacion' => 101, 'id_depto' => null], 'sasisopa' => ['id_estacion' => 101, 'id_depto' => null]]); $_SESSION['permisos_do'] = ['recursos-humanos' => ['descargar' => 1]]; $_SESSION['permisos'] = ['sgm' => ['descargar' => 1], 'sasisopa' => ['descargar' => 1]]; Session::set('csrf_handler_reached', false); exit('ok'); }
if ($path === '/__test/login-a-no-sgm') { Auth::forget(); Session::set('usuario', ['id' => 1, 'id_estacion' => 101, 'nombre_estacion' => 'Tenant A', 'razonsocial' => 'A SA', 'multiestacion' => true]); Session::set('module_context', ['sgm' => ['id_estacion' => 101, 'id_depto' => null], 'sasisopa' => ['id_estacion' => 101, 'id_depto' => null]]); $_SESSION['permisos'] = ['sasisopa' => ['descargar' => 1]]; exit('ok'); }
...
if ($path === '/__test/authorized-solicitud') { $solicitud = SolicitudChequeService::findAuthorizedSolicitudCheque((int) ($_GET['id'] ?? 0)); header('Content-Type: application/json'); exit(json_encode(['id' => $solicitud?->id])); }
if ($path === '/__test/solicitud-documentos') { header('Content-Type: application/json'); exit(json_encode(SolicitudChequeService::getDocumentos((int) ($_GET['id'] ?? 0)))); }
...
if ($path === '/requisitos-legales/download') { (new \App\Controllers\RequisitosLegalesController())->downloadRequisitoLegal(); exit; }
if ($path === '/sgm/normatividad-aplicable-mediciones/requisitos-legales/download') { (new \App\Controllers\SgmNormatividadController())->downloadRequisitoLegal(); exit; }
